...
export default function SearchIngredients({state, dispatch}) {
    ...
    async function callSearchIngredients2(query, page, doStoreSearch) {
        try {
    ...
    async function handleClick(e) {
        ...
        //setPage(1)
        //const call = (async() => {await callSearchIngredients2(state.query, 1, true)})()
        await callSearchIngredients2(state.query, 1, true)
    }

    async function handleIngredientPageChange(e, page) {
        console.log(`page change ${page}`)
        dispatch({type:ACTION_TYPES.SET_PAGE, payload:page})
        await callSearchIngredients2(state.prevQuery, page, false)
    }
